...
from classes.MenuCombatScrollableArea import MenuCombatScrollableArea
...
class Game:
    ...
    def __init__(self):
        ...
        # [REPERE 12 - 1]
        # LifeBar pour le self.player
        # [REPERE 12 - 2, 3] ds ce meme fichier
        self.player_life_bar = None
        ...
        # définit 1 canvas chargé d'afficher des infos sur les canvas de jeu.
        # pas encore utilisé.
        ...
        # Concerne le menu de test.
        self.menu01_buttons = [
            Button(820, 50, 100, 30, "QUIT", self.quit_program),
        ]
        ...
        # [REPERE 10 - 3]: Illustrat° des callbacks
        #       on annonce la methode qui est definie en tant que 'action'
        #       ds les parametres du constructeur ([REPERE 10 - 1])
        ...
        # [REPERE - 9 - 1]
        # Etablir 1 menu de combat. 1 scroll area composée de menus de combat.
        # Pour chaque monstre rencontré. Les menus de combat sont chacun
        # composé de 2 boutons: "attaquer" et "défendre" + le label du monstre.

        # [DESCRIPTION DE SITUATION]
        # Lorsqu'1 combat est crée, le monstre ne bouge plus. Il se bat
        # jusqu'à ce qu'il meurt ou tue le self.player.
        # Le self.player ne tue les monstres que les 1 après les autres.
        # Il n'y a pas (encore) de "mort groupée"

        # Fichiers:
        #   classes/MenuCombatScrollableArea.py
        #   classes/MenuCombat.py
        #       [REPERE - 9 - 2]:
        #       Verifie les collisions entre le self.player &
        #       les instances de classes/Monster.py
        #       C'est la methode check_collisions_with_monsters()

        #       [REPERE - 9 - 3]:
        #       Appel de la methode check_collisions_with_monsters()
        ...
        #       [REPERE - 9 - 6]:
        #       Les évènements boutons de chacun des menus combat des monstres

        #       [REPERE - 9 - 7]
        #       On nettoie les listes necessaires a la methode
        #       check_collisions_with_monsters()

        #       [REPERE - 9 - 8]
        #       On nettoie les listes necessaires a la methode
        #       check_collisions_with_monsters()

        # utilisées ds la methode check_collisions_with_monsters()
        # ms je ne sais pas si cela est vraiment de nécessaire
        # que la methode check_collisions_with_monsters() retourne 1 valeur.
        self.collision_detected = False

        # espacement vertical entre les menus de combat de chaque monstre.
        self.menu_height_menu_combat = 60

        # Instance de la scroll area ou sont affichés les menus de combat
        # de chaque monstre quand il y a collision avec le self.player
        self.samc = None

        # Booléen qui indique si l'instance self.samc a été initialisée ou pas.
        # Permet 1 seule et unique initialisation puisque la methode
        # check_collisions_with_monsters tourne en boucle.
        self.samc_initialisation = False

        # set qui contiendra les collisions de la frame précédente.
        # Nécessaire pr la MAJ des menus de combat
        self.previous_collisions = set()

        # Liste qui contiendra ...
        self.to_remove_monsters_menu_combat = []

        # Liste qui contiendra les instances de monstres qui peuvent
        # attaquer le self.player
        self.adversaires = []
    ...
    # Les boutons du menu général de test.

    # -------------------------------------------------------------------------

    # Label de ce bouton: "QUIT".
    # Quitte le programme.
    ...
    # [REPERE - 6 - 2]
    # Les méthodes du move menu qui permettent de diriger le self.player
    # en cliquant sur les fleches "haut", "droite", "bas" et "gauche"
    # TODO:
    #   On peut faire en sorte que ce move menu soit dessiné à la posit° du
    #   self.player afin que ce soit + pratique. Il faudra alors le dessiner
    #   sur son propre canvas

    # [REPERE 10 - 4]: Illustration des callbacks
    #       On déclare la methode qui est définie en tant que 'action'
    #       qd on a déclaré ds le def __init__ les MoveMenuButton
    #       (voir [REPERE 10 - 3])
    #       Ici player_move_right est 1 wrapper qui appelle la methode
    #       'va_a_droite()' de l'instance de classes/Player.py

    def player_move_right(self):
        """
        Déplace le joueur vers la droite en appelant la méthode va_a_droite()
        du joueur.

        Cette méthode sert de wrapper pour la méthode va_a_droite() du joueur.
        Elle s'assure que
        le joueur existe et que la nouvelle position est autorisée avant de
        mettre à jour la
        position du joueur.
    ...
    # [REPERE - 9 - 2]:
    def check_collisions_with_monsters(self):
        """
        Vérifie les collisions entre le joueur (self.player)
        et les monstres (self.liste_monsters).

        Attributs modifiés :
        - collision_detected_this_frame : Booléen indiquant si une
        collision a été détectée pendant cette frame.
        - liste_monsters : Liste des monstres dans le jeu.
        - previous_collisions : Ensemble des monstres qui étaient en
        collision avec le joueur dans la frame précédente.
        - samc : MenuCombatScrollableArea, le menu de combat.
        - samc_initialisation : Booléen indiquant si le menu de
        combat (samc) a été initialisé.
        - adversaires : Liste des monstres actuellement en collision
        avec le joueur.

        Retour :
        - Booléen : True si une collision est détectée, False sinon.

        Utilisation :
        Cette méthode est généralement appelée à chaque frame pour
        mettre à jour l'état du jeu.

        Exemple :
        >>> game.check_collisions_with_monsters()
        ...
        # Resetting the set for this frame
        collisions = set()

        # Technique slicing qui permet de ...?
        liste_monsters_copy = self.liste_monsters[:]

        #
        for monster in liste_monsters_copy:
            if self.player.position == monster.position:
                collision_detected_this_frame = True
                collisions.add(monster)
                monster.can_move = False

        # Initialize samc if not done
        if self.samc_initialisation is False and collisions:
            print("do nothing: initialisat° samc")
            self.samc = MenuCombatScrollableArea(
                self.screen,
                self.manager,
                (810, 300, 380, 300),
                (580, 800),
                self.menu_height_menu_combat)
            self.samc_initialisation = True

        if collisions:
            for monster in self.previous_collisions:
                if monster not in collisions:
                    print("maybe player moved")
                    print("maybe player killed this monster")
                    if monster in self.liste_monsters:
                        print("le monstre est tjs vivant, dc player moved")
                        monster.can_move = True
                        print(
                            "04: liste actualisée des monstres qui pvent attaquer le player")
                        self.adversaires.remove(monster)

                        if len(self.adversaires) > 0:
                            for m in self.adversaires:
                                print("04 " + str(m.name))
                            else:
                                print("04 pas d'adversaire")

                        print("----------------------------------------------")

                        self.to_remove_monsters_menu_combat.append(monster)
                        self.samc.remove_dead_monsters_02(
                            self.to_remove_monsters_menu_combat)
                        self.to_remove_monsters_menu_combat.clear()

                    else:
                        print("player killed monster")

                if monster in collisions:
                    if monster.life <= 0:
                        if monster in self.liste_monsters:
                            print(f"Le monstre {monster.name} est mort!")

                            # Supprime le monstre de self.adversaires
                            self.adversaires.remove(monster)

                            print(
                                "02: liste actualisée des monstres qui pvent attaquer le player")
                            if len(self.adversaires) > 0:
                                for m in self.adversaires:
                                    print("02 " + str(m.name))
                            else:
                                print("02 pas d'adversaire")

                            # Supprime le monstre de self.liste_monsters
                            # le fait disparaitre visuellement
                            self.liste_monsters.remove(monster)

                            # permet de mettre a jour le menu des combats
                            self.to_remove_monsters_menu_combat.append(monster)
                            self.samc.remove_dead_monsters_02(
                                self.to_remove_monsters_menu_combat)
                            self.to_remove_monsters_menu_combat.clear()
            ...
            for monster in collisions:
                if monster not in self.previous_collisions:
                    ...
                    # Supprime le monstre de self.adversaires
                    self.adversaires.append(monster)

                    print("monstres qui peuvent attaquer le player: self.adversaires")
                    if len(self.adversaires) > 0:
                        for m in self.adversaires:
                            print("01 " + str(m.name))
                    else:
                        print("01 self.adversaires est vide")

                    print("new collision, we need to add a specific option")
                    self.samc.add_combats_02(
                        self.player,
                        collisions,
                        self.menu_height_menu_combat)
        else:
            if self.samc and self.samc_initialisation:
                print("do nothing: destruct° samc")
                self.samc.kill()
                self.samc = None
                self.samc_initialisation = False
        ...
        #
        self.previous_collisions = collisions.copy()
    ...
    def process_player_movement(self,
        ...
        try:
            ...
            liste_fichiers_stairs = Utilitaires01.list_files(
                "monoplayer/save/stairs_json")
            ...
            # EXPLICAT° syntaxe:
            # if direct° == "next", alors on execute le
            #   FileProcessor.process_next(), sinon on
            #   execute le FileProcessor.process_previous()
            elts = (
            ...
            # S'il n'y a pas de fichier json précédent (c-a-d qd
            # le self.player est au premier stair et qu'il sort du jeu).
            # On doit afficher 1 nvelle fenetre.
            # S'il n'y a pas de prochain fichier json (c-a-d qd
            # le self.player est au dernier stair et qu'il sort du jeu).
            # On doit afficher 1 nvelle fenetre.
            if file_name == "No next element":
                print("Aucun prochain fichier JSON trouvé.")
                self.running = False
                pygame.quit()
                subprocess.run(
                    ["python3.9", "monoplayer/ExitGameFromLastStair.py"])
                sys.exit()
            elif file_name == "No previous element":
                print("Aucun fichier JSON précédent trouvé.")
                self.running = False
                pygame.quit()
                subprocess.run(
                    ["python3.9", "monoplayer/ExitGameFromFirstStair.py"])
                sys.exit()
            ...
            complete_path = os.path.join(
                'monoplayer/save/stairs_json', file_name)
            # complete_path
            ...
            Utilitaires01.update_json_file_player_multiple_keys(
                player_attributes)
            ...
            # [REPERE 12 - 3] creer la LifeBar de self.player
            if self.player and self.player_life_bar:
                # print("self.player_life_bar existe")
                # print("on la détruit")
                self.player_life_bar.kill()
                self.player_life_bar = LifeBar(
                    self.screen, 820, 200, self.player)
                self.player_life_bar.update()
            ...
            print("PROBLEME 07:")
            print(
                f"Erreur lors de la recherche du fichier JSON pour la direction {direction}.")
            print("Fichier: game.game.py")
            print("Methode: process_player_movement()")
        # Vous pouvez ajouter du code supplémentaire ici pour gérer l'erreur.
    ...
    def handle_events(self):
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame_gui.UI_BUTTON_PRESSED:
                ...
                # [REPERE - 9 - 6]
                for menu in self.samc.menu_combats:
                    if menu.button1 == event.ui_element \
                            or menu.button2 == event.ui_element:
                        menu.handle_button_press(event)
            ...
            if event.type == pygame.KEYUP:
                ...
                if event.key == pygame.K_SPACE:
                    if self.player is not None:
                        ...
                        # comparaison entre 1 tuple et 1 list,
                        # Il faut convertir l'1 des 2 ds le
                        # format de l'autre (ici comparaison de 2 list)
                ...
                if event.key == pygame.K_w:
                    print("self.liste_monsters")
                    for monster in self.liste_monsters:
                        print(monster.name)
                # -------------------------------------------------------------
                if event.key == pygame.K_x:
                    print("self.adversaires")
                    for monster in self.adversaires:
                        print(monster.name)
                # -------------------------------------------------------------
                if event.key == pygame.K_c:
                    print("self.to_remove_monsters_menu_combat")
                    for monster in self.to_remove_monsters_menu_combat:
                        print(monster.name)
                # -------------------------------------------------------------
    ...
    def update(self, time_delta):
        ...
        # [REPERE - 9 - 3]
        self.check_collisions_with_monsters()

        # =====================================================================

        # [REPERE - 9 - 4]
        # Permet aux instances de classes/Monster.py d'attaquer le self.player.
        # Permet de verifier si self.player est mort pdt 1 combat.
        if self.liste_monsters and self.samc_initialisation:
            for monster in self.adversaires[:]:
                # les monstres peuvent attaquer
                if self.player:
                    monster.update(time_delta, self.player)

            if self.player and self.player.life <= 0:
                print("Le joueur est mort!")
                self.is_running = False
                pygame.quit()
                subprocess.run(
                    ["python3.9", "monoplayer/ExitGameFromDeath.py"])
                sys.exit()
    ...
    def render(self):
        ...
        # [REPERE - 8 - 6]
        # Permet d'afficher le menu des combats
        # (peut-être les scrollbar)
        # (peut-etre le fond bleu)
        # peut-être d'autres choses (je ne sais plus)
        if self.samc and self.samc_initialisation is True:
            self.samc.draw()

        # [REPERE 12 - 2] MAJ de la LifeBar pour le self.player
        if self.player:
            self.player_life_bar = LifeBar(
                self.screen, 820, 200, self.player)
            self.player_life_bar.update()
    ...
    def run_game(self):
        ...
        for i in range(1, 11):
            ...
            self.rooms_tiles_array = self.get_rooms_tiles(
                self.white_tiles_array)
            self.attributes_rooms_array = self.get_attributes_rooms(
                self.white_tiles_array)
            ...
            if self.is_reachable():
                ...
                chosen_dict_to_set_exit_tile = random.choice(
                    self.white_tiles_array)
                ...
                chosen_dict_to_set_entry_tile = random.choice(
                    white_tiles_array_copy)
        ...
        if len(created_files) == 0:
            print("PROBLEME 02: Le tableau created_files ne contient pas d'elts.")
            print("Fichier: game/game.py")
            print("méthode: run_game()")
        ...
        if white_tiles_array_from_json \
            ...
            print("PROBLEME 01: Could not find valid start tile.")
            print("Fichier: game/game.py")
            print("méthode: run_game()")
            pygame.quit()
            sys.exit()
