...
class Monster:
    def __init__(self,
                 ...
                 radius: int = 8,
                 sprite_paths: Union[None, Dict[str, List[str]]] = None,
                 speed: int = 16,
        ...
        self.__sprite_paths = sprite_paths
        self.__speed = speed
        self.__name = name
        ...
        # Animation
        if sprite_paths:
            self.__animation_imgs = {
                direction: [pygame.transform.scale(pygame.image.load(path),
                                                   (TILE_SIZE, TILE_SIZE)) for path in paths]
    ...
    @property
    def sprite_paths(self) -> Union[None, Dict[str, List[str]]]:
        return self.__sprite_paths

    @sprite_paths.setter
    def sprite_paths(self, sprite_paths: Union[None, Dict[str, List[str]]]) -> None:
        self.__sprite_paths = sprite_paths
    ...
    def draw(self,
        ...
        if self.__animation_imgs:
            self.__surface.fill(TRANSPARENT_COLOR)
            img = self.__animation_imgs[self.__current_direction][self.__current_frame]

            # Calculate the top-left position to draw the image so that it
            # is centered at the Monster's position
            ...
            if self.__can_move:
                ...
                if self.__frame_count >= self.__frame_delay:
                    ...
                    self.__current_frame = (self.__current_frame + 1) \
                        % len(self.__animation_imgs[self.__current_direction])
        else:
